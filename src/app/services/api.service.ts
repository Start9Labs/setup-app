...
import { S9Notification, SSHFingerprint, ServerStatus, ServerModel, EmbassyConnection, AppMetrics, ServerMetrics, ServerSpecs } from '../models/server-model'
...
type TwoHundredOK = { never?: never } // hack for the unit type
...
interface ApiAppAvailablePreview extends ApiAppBase {
  versionLatest: string
  descriptionShort: string
}

interface ApiAppAvailableFull extends ApiAppAvailablePreview {
  releaseNotes: string
  descriptionLong: string
  versions: string[]
}

type ApiAppInstalled = ApiAppInstalled011 | ApiAppInstalled012

interface ApiAppInstalled011 extends ApiAppBase {
  versionLatest: string
  torAddress?: string
}

interface ApiAppInstalled012 extends ApiAppBase {
  torAddress: string
}

type ApiServer = ApiServer011 | ApiServer012

interface ApiServer011 {
  status: ServerStatus
  versionLatest: string
  versionInstalled: string
}

interface ApiServer012 {
  status: ServerStatus
  versionInstalled: string
