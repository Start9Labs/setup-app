...
export class APService {
  ...
  async registerPubkey (publicKey: string): Promise<void> {
    return this.request(Method.post, this.url + '/device', undefined, {
      publicKey,
    })
  }

  async getTorAddress (): Promise<string> {
    return this.request(Method.get, this.url + '/tor')
  }

  async submitWifiCredentials (ssid: string, password: string): Promise<void> {
    return this.request(Method.post, this.url + '/wifi', undefined, {
      ssid,
      password,
    })
  }
  ...
  async enableWifi (ssid: string): Promise<void> {
    return this.request(Method.post, this.url + '/wifi/enable', undefined, {
      ssid,
    })
  }
